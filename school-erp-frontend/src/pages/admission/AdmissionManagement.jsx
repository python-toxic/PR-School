...
import { 
  ...
  CheckSquare, CreditCard, Eye, Edit2, Trash2, QrCode as QRCode, Copy, Clipboard
} from 'lucide-react';
...
const STAGES = [
  { id: 1, name: 'Student Details', icon: User, progress: 16 },
  { id: 2, name: 'Parent/Guardian', icon: Users, progress: 32 },
  { id: 3, name: 'Address Details', icon: MapPin, progress: 48 },
  { id: 4, name: 'Documents Upload', icon: FileText, progress: 64 },
  { id: 5, name: 'Payment', icon: CreditCard, progress: 80 },
  { id: 6, name: 'Review & Submit', icon: CheckSquare, progress: 100 }
];
...
export default function AdmissionManagement() {
  ...
  // Listen for class updates from Settings or Classes module
  useEffect(() => {
    const handleClassUpdate = () => {
      loadClasses();
    };
    
    window.addEventListener('classes-updated', handleClassUpdate);
    window.addEventListener('school-settings-updated', handleClassUpdate);
    
    return () => {
      window.removeEventListener('classes-updated', handleClassUpdate);
      window.removeEventListener('school-settings-updated', handleClassUpdate);
    };
  }, []);

  // Compute class fee when class changes
  useEffect(() => {
    const amt = getClassFee(studentData.classApplyingFor);
    setPaymentData(prev => ({
      ...prev,
      feeAmount: amt || 0,
      paidAmount: 0,
    ...
    try {
      ...
      try {
        ...
        if (data.success && data.classes.length > 0) {
          // Get selected classes from settings to filter
          ...
          if (savedSettings) {
            try {
              ...
              selectedClassNames = settings.selectedClasses || [];
            } catch (error) {
          ...
          data.classes.forEach(cls => {
            // Only include if selected in settings (or if no selections, include all)
            if (selectedClassNames.length === 0 || selectedClassNames.includes(cls.fullName)) {
              ...
              if (!classMap[cls.name].sections.includes(cls.section)) {
                classMap[cls.name].sections.push(cls.section);
              }
            }
          });
      ...
      if (saved) {
        ...
        // Group by class name and collect sections
        const classMap = {};
        list.forEach(c => {
          ...
          if (c.section && !classMap[name].sections.includes(c.section)) {
            classMap[name].sections.push(c.section);
          }
        });
      ...
      setClasses([
        { className: 'Nursery', sections: ['A'] },
        { className: 'LKG', sections: ['A'] },
        { className: 'UKG', sections: ['A'] },
        { className: 'Class 1', sections: ['A'] },
        { className: 'Class 2', sections: ['A'] },
        { className: 'Class 3', sections: ['A'] },
        { className: 'Class 4', sections: ['A'] },
        { className: 'Class 5', sections: ['A'] },
        { className: 'Class 6', sections: ['A'] },
        { className: 'Class 7', sections: ['A'] },
        { className: 'Class 8', sections: ['A'] },
        { className: 'Class 9', sections: ['A'] },
        { className: 'Class 10', sections: ['A'] },
      ]);
    } catch (err) {
  ...
  useEffect(() => {
    const cls = classes.find(c => c.className === studentData.classApplyingFor);
    setSections(cls?.sections || []);
  }, [studentData.classApplyingFor, classes]);

  // Get class fee from localStorage or defaults
  const getClassFee = (className) => {
    try {
      const mapStr = localStorage.getItem('class-fees');
      if (mapStr) {
        const feeMap = JSON.parse(mapStr);
        const val = feeMap[className];
  ...
  const updateDocumentName = (docId, customName) => {
    setDocuments(prev => prev.map(d => d.id === docId ? { ...d, customDocName: customName } : d));
  };
  ...
  const validateStage4 = () => {
    const birthCert = documents.find(d => 
      (d.customDocName && d.customDocName.toLowerCase().includes('birth')) ||
      (d.type && d.type.toLowerCase().includes('birth'))
    );
    if (!birthCert) {
      alert('Birth Certificate is mandatory');
      return false;
    }
    // Generate credentials: username = parent mobile, password = DOB (DDMMYYYY)
    const parentMobile = parentData.fatherMobile || parentData.motherMobile || parentData.guardianContact || '';
    if (parentMobile && studentData.dob) {
      const dob = new Date(studentData.dob);
      const pad = (n) => String(n).padStart(2, '0');
      const password = `${pad(dob.getDate())}${pad(dob.getMonth()+1)}${dob.getFullYear()}`;
      setLoginCredentials({ username: parentMobile, password });
    }
    return true;
  };

  const validateStage5 = () => {
    if (!studentData.classApplyingFor) {
      alert('Please select class before payment');
      return false;
    }
    if (!paymentData.feeAmount || paymentData.feeAmount <= 0) {
      alert('Class fee is not available for the selected class');
      return false;
    }
  ...
  const handleNext = async () => {
    ...
    if (currentStage === 5 && !validateStage5()) return;
  ...
  const saveDraft = async () => {
    ...
    localStorage.setItem(`admission_draft_${admissionId}`, JSON.stringify({
      ...
      parentData,
      addressData,
      documents: documents.map(d => ({ id: d.id, name: d.name, customDocName: d.customDocName })),
      paymentData
    }));
  };
  ...
  const handleFinalSubmit = async () => {
    setSubmitting(true);
    try {
      // Credentials for print: parent's mobile as username, DOB (DDMMYYYY) as password
      const username = loginCredentials.username || parentData.fatherMobile || parentData.motherMobile || parentData.guardianContact || '';
      let password = loginCredentials.password;
      if (!password && studentData.dob) {
        const d = new Date(studentData.dob);
        const p = (n) => String(n).padStart(2,'0');
        password = `${p(d.getDate())}${p(d.getMonth()+1)}${d.getFullYear()}`;
      }

      // Build request body matching backend contract
      const reqBody = {
        student: {
          name: `${studentData.firstName} ${studentData.middleName || ''} ${studentData.lastName}`.replace(/\s+/g,' ').trim(),
          mobile: studentData.contactNumber,
          dob: studentData.dob,
          gender: studentData.gender,
          bloodGroup: studentData.bloodGroup || undefined,
          address: `${addressData.currentLine1}${addressData.currentLine2 ? ', ' + addressData.currentLine2 : ''}`,
          city: addressData.currentCity,
          state: addressData.currentState,
          pincode: addressData.currentPincode,
          classId: studentData.classApplyingFor,
          section: studentData.section,
          academicYear: studentData.academicYear,
          profileImageUrl: studentPhotoPreview || undefined,
        },
        parent: {
          name: parentData.fatherName,
          mobile: parentData.fatherMobile,
          email: parentData.fatherEmail || undefined,
          fatherName: parentData.fatherName,
          motherName: parentData.motherName,
          guardianName: parentData.guardianName || undefined,
        },
        fee: {
        ...
        documents: documents.map(d => ({
          documentType: d.customDocName || d.type || 'Document',
          documentName: d.name,
          documentUrl: d.preview || '',
        })),
      };
      ...
      // Auto-save parent to localStorage for Parent/Guardian Management
      const parentCard = {
        id: `parent_${Date.now()}`,
        name: parentData.fatherName,
        mobile: parentData.fatherMobile,
      ...
      try {
        ...
        if (saved) savedParents = JSON.parse(saved);
      } catch {}
      savedParents.push(parentCard);
      localStorage.setItem('admissions-parents', JSON.stringify(savedParents));
          ...
          <div class="section">
            ...
            <div style="padding-left: 15px;">
              ${documents.length > 0 ? documents.map(doc => `
                <div style="padding: 5px 0;">📄 ${doc.customDocName || doc.type || 'Document'} - ${doc.name}</div>
          ...
          <div class="section">
            ...
            <div class="info-grid">
              <div class="info-item">
                ...
                <div class="info-value">₹ ${paymentData.feeAmount?.toLocaleString() || 0}</div>
              </div>
              <div class="info-item">
                <div class="info-label">Payment Mode</div>
                <div class="info-value">${paymentData.paymentMode || 'N/A'}</div>
              </div>
              <div class="info-item">
                <div class="info-label">Discount</div>
                <div class="info-value">₹ ${paymentData.discount?.toLocaleString() || 0}</div>
              </div>
              <div class="info-item">
                <div class="info-label">Paid Amount</div>
                <div class="info-value">₹ ${paymentData.paidAmount?.toLocaleString() || 0}</div>
              </div>
              <div class="info-item">
                <div class="info-label">Due Amount</div>
                <div class="info-value">₹ ${paymentData.dueAmount?.toLocaleString() || 0}</div>
              </div>
              <div class="info-item">
                <div class="info-label">Status</div>
                <div class="info-value">${paymentData.status === 'paid' ? 'Paid' : 'Pending'}</div>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">Create new admissions with 5-stage wizard process</p>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                ...
                Generate QR for Parents
              </h3>
              <p className="text-sm text-gray-600 mb-4">
                Parents scan this QR code to open the admission form, submit online, and the application is auto-created in the system.
              </p>
        ...
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">New Admission Form</h2>
          <p className="text-gray-600">Complete all 5 stages to enroll the student</p>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-sm text-red-800">
                  <strong>⚠️ Important:</strong> Birth Certificate is mandatory. Please upload it.
                </p>
              </div>
              ...
              <div className="space-y-3">
                {documents.map((doc, idx) => (
                  <div key={doc.id} className="bg-gray-50 border rounded-lg p-4">
                    ...
                    <div className="pl-16">
                      ...
                      <input 
                        type="text"
                        placeholder="e.g., Birth Certificate, Aadhaar, School TC, Report Card, etc."
                        value={doc.customDocName || ''}
                        onChange={(e) => updateDocumentName(doc.id, e.target.value)}
                        className="w-full px-2 py-1.5 border border-gray-300 rounded text-sm"
                      />
                      <p className="text-xs text-gray-400 mt-1">Enter any document name you want to upload</p>
                    </div>
                  </div>
                ))}
                {documents.length === 0 && (
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
                  ...
                  <p className="text-xl font-bold text-blue-700">{studentData.classApplyingFor || '—'}</p>
                </div>
                <div className="bg-green-50 rounded-xl p-4 border border-green-200">
                  <p className="text-sm text-gray-600">Class Fee</p>
                  <p className="text-xl font-bold text-green-700">₹ {paymentData.feeAmount?.toLocaleString() || 0}</p>
                </div>
                <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                  <p className="text-sm text-gray-600">Due Amount</p>
                  <p className="text-xl font-bold text-amber-700">₹ {paymentData.dueAmount?.toLocaleString() || 0}</p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3 mt-7">
                  <input 
                    type="checkbox" 
                    id="mark-paid"
                    checked={paymentData.status === 'paid'}
                    onChange={(e) => {
                      const isPaid = e.target.checked;
                      setPaymentData(prev => ({
                        ...prev,
                        status: isPaid ? 'paid' : 'pending',
                        paidAmount: isPaid ? prev.feeAmount - (prev.discount || 0) : 0,
                        dueAmount: isPaid ? 0 : (prev.feeAmount - (prev.discount || 0)),
                      }));
                    }}
                    className="w-4 h-4"
                  />
                  <label htmlFor="mark-paid" className="text-sm text-gray-700">Mark as Paid</label>
                </div>
              </div>
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      setPaymentData(prev => ({
                        ...prev,
                        discount: val,
                        dueAmount: Math.max(0, (prev.feeAmount - val - (prev.status === 'paid' ? (prev.feeAmount - val) : prev.paidAmount)))
                      }));
                    }}
                    className={inputCls}
                    placeholder="e.g., 500"
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      setPaymentData(prev => ({
                        ...prev,
                        paidAmount: val,
                        status: val >= (prev.feeAmount - (prev.discount || 0)) ? 'paid' : 'pending',
                        dueAmount: Math.max(0, (prev.feeAmount - (prev.discount || 0) - val))
                      }));
                    }}
                    className={inputCls}
              ...
              <div className="flex justify-between pt-4 border-t">
                ...
                <button
                  onClick={handleNext}
                  className="flex items-center gap-2 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold shadow-sm"
                >
                  Next
          ...
          {currentStage === 6 && (
            <div className="space-y-6">
              ...
              <div className="bg-green-50 rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-gray-600">Fee Amount:</span> <span className="font-semibold">₹ {paymentData.feeAmount?.toLocaleString() || 0}</span></div>
                  <div><span className="text-gray-600">Payment Mode:</span> <span className="font-semibold">{paymentData.paymentMode || 'N/A'}</span></div>
                  <div><span className="text-gray-600">Discount:</span> <span className="font-semibold">₹ {paymentData.discount?.toLocaleString() || 0}</span></div>
                  <div><span className="text-gray-600">Paid Amount:</span> <span className="font-semibold">₹ {paymentData.paidAmount?.toLocaleString() || 0}</span></div>
                  <div><span className="text-gray-600">Due Amount:</span> <span className="font-semibold">₹ {paymentData.dueAmount?.toLocaleString() || 0}</span></div>
                  <div><span className="text-gray-600">Status:</span> <span className={`font-semibold ${paymentData.status === 'paid' ? 'text-green-700' : 'text-amber-700'}`}>{paymentData.status === 'paid' ? 'Paid' : 'Pending'}</span></div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {documents.map(doc => (
                    <div key={doc.id} className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium">{doc.customDocName || doc.type || 'Document'}</span>
